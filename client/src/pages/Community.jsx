...
const Community = () => {
  ...
  const fetchImages=async ()=> {
    try {
      const {data}=await axios.post('/api/user/published-images')
      if (data.success){
  ...
  return (
    ...
    overflow-y-scroll'>
      ...
      dark:text-blue-100'>Community Images</h2>
      
      {images.length > 0 ? (
        <div className='flex flex-wrap max-sm:justify-center gap-5'>
          {images.map((item, index)=>(
            <a key={index} href={item.imageUrl} target='_blank' className='relative group block rounded-lg overflow-hidden
            border border-gray-200 dark:border-blue-700 shadow-sm hover:shadow-md transition-shadow duration-300'>
              <img src={item.imageUrl} alt="" className='w-full h-40 md:h-52 
